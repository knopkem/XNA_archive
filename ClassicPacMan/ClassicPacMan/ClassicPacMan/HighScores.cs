...
namespace ClassicPacMan
{
    ...
    public class HighScores : DrawableGameComponent
    {
        ...
        {
            base.Draw(gameTime);
            var position = new Vector2(graphics_.PreferredBackBufferWidth/2 - 150,
                                       graphics_.PreferredBackBufferHeight/2 - 200);
            spriteBatch_.Begin();
            for (int i = 0; i < 10; i++)
            {
                spriteBatch_.DrawString(scoreFont_, (i + 1) + ".", new Vector2(position.X, position.Y + (30*i)),
                                        Color.White);
                if (i < scores_.Count)
                {
                    spriteBatch_.DrawString(scoreFont_, scores_[i], new Vector2(position.X + 50, position.Y + (30*i)),
                                            Color.White);
                }
            }
