...
namespace ClassicPacMan
{
    ...
    public class Ghost
    {
        private readonly Color colorBase_;
        private readonly Texture2D eyesBase_;
        private readonly Texture2D eyesCenter_;
        private readonly Texture2D ghostBase1_;
        private readonly Texture2D ghostBase2_;
        private readonly Texture2D ghostChased_;
        private readonly Ghosts identity_;
        private readonly int previousNumCrumps_;
        private readonly List<Point> scatterTiles_;
        ...
        private void AIScatter()
        {
            ...
            // Scatter AI may be overridden by certain tiles
            if (AIOverride())
            {
                return;
            }
            ...
            int favoredTile = CfExtensions.FindIndex(scatterTiles_, i => i == position_.Tile);
        ...
        private void AttackAIInky()
        {
            Tile nextTile = NextTile(player_.Direction, player_.Position);
            Tile nextNextTile = NextTile(player_.Direction, new Position(nextTile.ToPoint, Point.Zero));
            var line = new Vector2(blinky_.Position.Tile.X - nextNextTile.ToPoint.X,
                                   blinky_.Position.Tile.Y - nextNextTile.ToPoint.Y);
            line *= 2;
        ...
        private Direction FindDirection(Point destination)
        {
            ...
            else
            {
                ...
                return CfExtensions.Find(directions, i => NextTile(i).IsOpen);
            }
        }
        ...
        public void Draw(GameTime gameTime, Vector2 boardPosition)
        {
            ...
            var eyesBasePosition = new Vector2(0, 0);
            ...
            if (State == GhostState.Blue)
            {
                ...
                else
                {
                    ...
                    RenderSprite(wiggle_ ? ghostBase1_ : ghostBase2_, null, boardPosition, position,
                                 flash ? Color.Blue : Color.White);
                                  ...
                                  Color color)
        {
            ...
            {
                var deltaPixel = (int) ((position.X + textureWidth) - (boardPosition.X + (16*28)));
                    // Number of pixels off the board
