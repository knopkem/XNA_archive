...
namespace ClassicPacMan
{
    ...
    public class Player
    {
        private readonly Texture2D dyingFrames_;
        private readonly Texture2D[] eatingFrames_;
        ...
        private void TryMove()
        {
                     ...
                     (position_.Tile == new Point(27, 14) && direction_ == Direction.Right))
            {
                ...
                // If on a tile, we only move if the next tile in our direction is open
                      ...
                      Grid.TileGrid[position_.Tile.X, position_.Tile.Y - 1].Type == TileTypes.Open) ||
                     (direction_ == Direction.Down &&
                      Grid.TileGrid[position_.Tile.X, position_.Tile.Y + 1].Type == TileTypes.Open) ||
                     (direction_ == Direction.Left &&
                      Grid.TileGrid[position_.Tile.X - 1, position_.Tile.Y].Type == TileTypes.Open) ||
        ...
        public void Draw(GameTime gameTime, Vector2 boardPosition)
        {
            ...
            position.X = boardPosition.X + (position_.Tile.X*16) + position_.DeltaPixel.X -
                         ((eatingFrames_[0].Width - 16)/2);
            position.Y = boardPosition.Y + (position_.Tile.Y*16) + position_.DeltaPixel.Y -
                         ((eatingFrames_[0].Height - 16)/2);
        ...
        private void RenderSprite(Texture2D spriteSheet, Rectangle? rectangle, Vector2 boardPosition, Vector2 position)
        {
            ...
            {
                var deltaPixel = (int) ((position.X + 26) - (boardPosition.X + (16*28)));
