    ...
    public class GameLoop : DrawableGameComponent
    {
        ...
        public int Score
        {
            ...
            private set
            {
                ...
                {
                    soundBank_.PlayCue(SoundBank.SoundTitle.ExtraLife);
                    xlives_++;
                }
                score_ = value;
            }
        ...
        public override void Initialize()
        {
            ...
            ghosts_ = new List<Ghost>
                          {
                              new Ghost(Game, player_, Ghosts.Blinky),
                              new Ghost(Game, player_, Ghosts.Clyde),
                              new Ghost(Game, player_, Ghosts.Inky),
                              new Ghost(Game, player_, Ghosts.Pinky)
                          };
            ghosts_[2].SetBlinky(ghosts_[0]); // Oh, dirty hack. Inky needs this for his AI.
            soundBank_.PlayCue(SoundBank.SoundTitle.Intro);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            CfExtensions.RemoveAll(scoreEvents_, i => DateTime.Now - i.When > TimeSpan.FromSeconds(5));
            ...
            if (noCrumpsLeft())
            {
                if (Constants.Level < 21)
                {
                    ...
                    soundBank_.PlayCue(SoundBank.SoundTitle.NewLevel);
            ...
            if (player_.Position.DeltaPixel == Point.Zero)
            {
                ...
                {
                    soundBank_.PlayCue(paChomp_ ? SoundBank.SoundTitle.PacManEat1 : SoundBank.SoundTitle.PacManEat2);
                    paChomp_ = !paChomp_;
                    Score += 10;
                 ...
                 (player_.Position.Tile.X == 14 && player_.Position.DeltaPixel.X == -8)))
            {
                ...
                soundBank_.PlayCue(SoundBank.SoundTitle.FruitEat);
                bonusPresent_ = false;
            ...
            foreach (Ghost ghost in ghosts_)
            {
                ...
                if (!Rectangle.Intersect(playerArea, ghostArea).IsEmpty)
                {
                    ...
                    {
                        GhostSoundsManager.StopLoops();
                        soundBank_.PlayCue(SoundBank.SoundTitle.EatGhost);
        ...
        private void KillPacMan()
        {
            ...
            GhostSoundsManager.StopLoops();
            soundBank_.PlayCue(SoundBank.SoundTitle.Death);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            {
                spriteBatch_.Draw(
                    ((DateTime.Now.Second*1000 + DateTime.Now.Millisecond)/350)%2 == 0 ? board_ : boardFlash_,
                    boardPosition, Color.White);
            ...
            {
                spriteBatch_.Draw(xlife_,
                                  new Vector2(boardPosition.X + 10 + (20*i), board_.Height + boardPosition.Y + 10),
                                  Color.White);
            }

            // Draw current score
            spriteBatch_.DrawString(scoreFont_, "SCORE", new Vector2(boardPosition.X + 30, boardPosition.Y - 50),
                                    Color.White);
            spriteBatch_.DrawString(scoreFont_, Score.ToString(),
                                    new Vector2(boardPosition.X + 30, boardPosition.Y - 30), Color.White);

            // Draw current level
            spriteBatch_.DrawString(scoreFont_, "LEVEL",
                                    new Vector2(boardPosition.X + board_.Width - 80, boardPosition.Y - 50), Color.White);
            spriteBatch_.DrawString(scoreFont_, Constants.Level.ToString(),
                                    new Vector2(boardPosition.X + board_.Width - 80, boardPosition.Y - 30), Color.White);

            // Draw a bonus fruit if any
            if (bonusPresent_)
            {
                spriteBatch_.Draw(bonus_[Constants.BonusSprite()],
                                  new Vector2(boardPosition.X + (13*16) + 2, boardPosition.Y + (17*16) - 8), Color.White);
            }

            // Draw captured bonus fruits at the bottom of the screen
            int k = 0;
            foreach (var kvp in bonusEaten_)
            {
                for (int i = 0; i < kvp.Value; i++)
                {
                    spriteBatch_.Draw(bonus_[kvp.Key],
                                      new Vector2(boardPosition.X + 10 + (22*(k + i)),
                                                  board_.Height + boardPosition.Y + 22), Color.White);
                }
                k += kvp.Value;
            }
            ...
            {
                spriteBatch_.DrawString(scoreEventFont_, se.Score.ToString(),
                                        new Vector2(
                                            boardPosition.X + (se.Position.Tile.X*16) + se.Position.DeltaPixel.X + 4,
                                            boardPosition.Y + (se.Position.Tile.Y*16) + se.Position.DeltaPixel.Y + 4),
                                        Color.White);
            }
            ...
            {
                spriteBatch_.DrawString(scoreFont_, "GET READY!",
                                        new Vector2(boardPosition.X + (board_.Width/2) - 58, boardPosition.Y + 273),
                                        Color.Yellow);
            }
